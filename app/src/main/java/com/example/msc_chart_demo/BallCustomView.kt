...
//      canvas.drawLine(startX, height.toFloat(), startX + width, startY, Const.guideLineBlue)
      ...
      canvas.drawCircle(centerX, centerY, ballRadius*0.995f, Const.ballColors[0])
      ...
      canvas.drawText(text.clubTypeText, centerX, centerY + distance, textPaint)



      // 가운데 격자선
      if (centerGridLine) {
         canvas.drawLine(centerX, startY, centerX, startY + height, Const.guideLineBlue)
         canvas.drawLine(startX, centerY, startX + width, centerY, Const.guideLineBlue)
      }
   }
